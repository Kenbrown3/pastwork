...
Q1e: Try an inequality like x > 2*y. Explain the difierence between the following functions
x > y*2 and
(x > y)*2
...
The difference between x > y2 and (x > y)2 is that x>y2 tests the values of x against twice the values of y, while (x>y)*2 tests if an element of x is greater than the corresponding value of y then doubles the binary results of that test.
...
hist(compensation, main="Histogram of Compensation in a Sample of Counties",
     xlab="Compensation")
```
...
```{r}
compensation <- scan("compensation.txt",n=170,skip=3)
hist(compensation, main="Histogram of Compensation in a Sample of Counties with Breaks=35",
     breaks=35, col="blue3", xlab="Compensation")
```
```{r}
compensation <- scan("compensation.txt",n=170,skip=3)
hist(compensation, main="Histogram of Compensation in a Sample of Counties with Breaks=100",
     breaks=100, col="blue3", xlab="Compensation")
```
...
This is summing the number of observations bellow 50000. We can use this result to form an estimate for the probability a given county has a compensation rate below 50000 in addition to a workable chi-squared test statistic.   
